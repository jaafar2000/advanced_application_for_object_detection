...
cal = dict([(x,cal_range/dm) for x in range(1,int(dm)+1,pixel_base)])
...
while 1:
    ...
    #-------------------------------
        ...
        for c in contours:
            ...
            # add dimensions
            draw.add_text(frame0,f'{xlen:.2f}mm',x1-((x1-x2)/2),min(y1,y2)-8,center=True,color='red')
            draw.add_text(frame0,f'Area: {carea:.2f}mm^2',x3,y2+8,center=True,top=True,color='red')
            if alen:
                draw.add_text(frame0,f'Avg: {alen:.2f}mm',x3,y2+34,center=True,top=True,color='green')
            if x1 < width-x2:
                draw.add_text(frame0,f'{ylen:.2f}mm',x2+4,(y1+y2)/2,middle=True,color='red')
            else:
                draw.add_text(frame0,f'{ylen:.2f}mm',x1-4,(y1+y2)/2,middle=True,right=True,color='red')
